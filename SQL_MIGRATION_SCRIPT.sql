...
-- users 마이그레이션
-- ------------------------------------------------------------
INSERT INTO users (id, email, password_hash, name, student_id, role, provider, provider_id, profile_image, created_at)
SELECT 
  (value->>'id')::UUID,
  value->>'email',
  value->>'password',  -- TODO: 나중에 bcrypt로 해싱 필요
  value->>'name',
  value->>'studentId',
  COALESCE(value->>'role', 'user'),
  COALESCE(value->>'provider', 'local'),
  value->>'kakaoId',  -- provider_id
  value->>'profileImage',
  COALESCE((value->>'createdAt')::TIMESTAMP WITH TIME ZONE, NOW())
FROM kv_store_ce976ffa
...
-- notices 마이그레이션
-- ------------------------------------------------------------
INSERT INTO notices (id, title, content, category, priority, author_id, created_at, updated_at)
SELECT 
  (value->>'id')::UUID,
  value->>'title',
  value->>'content',
  COALESCE(value->>'category', 'general'),
  COALESCE(value->>'priority', 'medium'),
  (value->>'authorId')::UUID,
  COALESCE((value->>'createdAt')::TIMESTAMP WITH TIME ZONE, NOW()),
  COALESCE((value->>'updatedAt')::TIMESTAMP WITH TIME ZONE, NOW())
FROM kv_store_ce976ffa
WHERE key LIKE 'notice:%'
  AND (value->>'id') IS NOT NULL
  AND (value->>'authorId') IS NOT NULL
ON CONFLICT (id) DO NOTHING;
