...
// 직행버스 정류장 (정문 → SCH아트홀 → 천안터미널 방향 예시)
const DIRECT_STOPS = [
  { name: "정문",       lat: 36.769014, lng: 126.927978 },
...
export default function AdminDashboard() {
  ...
  // ── Polling: refresh bus list every 10s on buses tab ──
  useEffect(() => {
  ...
  const startTestBus = async () => {
    ...
    try {
      ...
      if (existing) {
        ...
        try {
          const bus = await api.createBus({ name: busName, type: testBusType === "campus" ? "campus" : "direct" });
          busId = bus.id;
          await api.updateBus(busId, { status: "active" });
        } catch (createErr: any) {
      ...
      <div className="p-8">
        ...
        {activeTab === "buses" && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-['Public_Sans'] font-semibold text-[#0f172a] text-[18px]">
                  실시간 버스 현황
                </h3>
                <button
                  onClick={fetchBuses}
              ...
              {busesLoading && buses.length === 0 ? (
                <div className="space-y-3">
                  {Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="animate-pulse flex items-center gap-4 p-4 rounded-lg border border-gray-100">
                    ...
                    return (
                      <div
                        ...
                        className={`flex items-center gap-4 p-4 rounded-xl border transition-all ${
                          isActive ? "border-[#1e3b8a]/20 bg-[#1e3b8a]/3" : "border-gray-100 bg-gray-50/50"
                        }`}
                      >
                        {/* Status icon */}
                        <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                          isActive ? "bg-[#1e3b8a]" : "bg-gray-200"
                        }`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className="font-['Public_Sans'] font-semibold text-[#0f172a] text-[15px] truncate">
                              {bus.name}
                            </span>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          {isActive && (
                            <>
                              {forceStopConfirm === bus.id ? (
                                <div className="flex items-center gap-2">
                                  <span className="font-['Public_Sans'] text-[12px] text-[#64748b]">강제 종료?</span>
                                  <button
                          ...
                          {deleteConfirm === bus.id ? (
                            <div className="flex items-center gap-2">
                              <span className="font-['Public_Sans'] text-[12px] text-[#64748b]">삭제?</span>
                              <button
            ...
            {/* ② 버스 등록 */}
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              <h3 className="font-['Public_Sans'] font-semibold text-[#0f172a] text-[18px] mb-1">
                버스 등록
              </h3>
              <p className="font-['Public_Sans'] text-[#94a3b8] text-[13px] mb-5">
                새 버스를 시스템에 등록합니다. 등록된 버스는 기사 앱에서 선택할 수 있습니다.
              </p>
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
              ...
              <p className="font-['Public_Sans'] text-[#94a3b8] text-[13px] mb-5">
                가상 버스를 실제 경로로 이동시켜 지도 연동을 테스트합니다. 테스트 완료 후 버스는 자동으로 미운행 상태로 전환됩니다.
              </p>
