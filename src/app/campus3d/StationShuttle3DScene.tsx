...
function StationWorld(props: StationShuttle3DSceneProps) {
  ...
  return (
    <>
      ...
      {stops.map((stop, index) => (
        <group key={stop.id} position={[stop.position[0], getStationTerrainHeight(stop.position[0], stop.position[1]) + 2, stop.position[1]]} onClick={(event) => { event.stopPropagation(); props.onSelectStop(stop); }}>
          <mesh castShadow>
            ...
            <meshStandardMaterial color="#ffffff" emissive="#f59e0b" emissiveIntensity={0.12} />
          </mesh>
      ...
      <OrbitControls ref={controls} makeDefault enableDamping dampingFactor={0.075} minDistance={80} maxDistance={3400} minPolarAngle={0.2} maxPolarAngle={Math.PI / 2.08} screenSpacePanning={false} />
