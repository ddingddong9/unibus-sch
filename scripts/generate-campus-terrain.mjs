...
function unproject(x, z) {
  return {
    lat: ORIGIN.lat - z / 110_540,
    lng: ORIGIN.lng + x / metersPerLongitude,
  };
}
...
for (let row = 0; row < RESOLUTION; row += 1) {
  ...
  for (let column = 0; column < RESOLUTION; column += 1) {
    ...
    rowSamples.push(sampleElevation(x, z));
  }
  rawHeights.push(...await Promise.all(rowSamples));
}
...
const outputPath = path.join(root, "src/app/campus3d/terrain-data.json");
await writeFile(outputPath, `${JSON.stringify(output)}\n`, "utf8");
console.log(`Generated ${RESOLUTION}x${RESOLUTION} terrain grid from ${tileCache.size} elevation tiles.`);
console.log(`Elevation range ${sourceMin.toFixed(1)}m - ${sourceMax.toFixed(1)}m, base ${baseElevation.toFixed(1)}m.`);
